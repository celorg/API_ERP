...
class EmployeePermission(permissions.BasePermission):
    massage = 'O funcionário não tem permissão para gerenciar os funcionários'
...
class GroupsPermission(permissions.BasePermission):
    massage = 'O funcionário não tem permissão para gerenciar os grupos'
...
class GroupsPermissionsPermission(permissions.BasePermission):
    massage = 'O funcionário não tem permissão para gerenciar as permissiões'

    def has_permission(self, request, _view):
        return check_permissions(request.user, request.method, permission_to='permission')
    
class TaskPermission(permissions.BasePermission):
    message = 'O funcionário não tem permissão parar gerenciar as tarefas de todos os funcionários'
