...
from companies.utils.permissions import EmployeePermission, Group_Permissions
...
class Employees(Base):
    ...
    def get(self, request):
        ...
        # Precisa obter o dono da empresa
    ...
    def post(self, request):
        ...
        enterprise_id = self.get_enterprise_id(request.user.id)
        sign_user = Authentication.signup(
            self,
            name,
            email,
            password,
        ...
        if isinstance(sign_user, User):
            return Response({'success': True}, status=status.HTTP_201_CREATED)
        
        return Response(sign_user, status=status.HTTP_400_BAD_REQUEST)
